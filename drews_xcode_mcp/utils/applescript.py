...
def build_open_and_wait_applescript(escaped_path: str, escaped_scheme: Optional[str] = None) -> str:
    ...
    return (
        ...
        f'    -- `first workspace document whose path is ...` raises rather than\n'
        f'    -- returning missing value when there is no match, so the lookup\n'
        f'    -- must be wrapped in a try -- otherwise a not-yet-open project would\n'
        f'    -- always error here instead of falling through to `open`.\n'
        f'    set workspaceDoc to missing value\n'
        f'    try\n'
